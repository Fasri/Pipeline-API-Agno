...
def ler_dados_postgres():
    ...
    try:
        ...
        query = "SELECT * FROM precos ORDER BY timedata DESC"
...
def main():
    ...
    if not df.empty:
        ...
        df['timestamp'] = pd.to_datetime(df['timedata'])
        df = df.sort_values(by='timedata')
        
        st.subheader("📈 Evolução do Preço do Bitcoin")
        st.line_chart(data=df, x='timedata', y='valor', use_container_width=True)
